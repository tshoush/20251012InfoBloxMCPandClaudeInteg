#!/usr/bin/env python
"""
Interactive chat interface for Claude AI with RAG-enhanced InfoBlox knowledge
Uses vector database for semantic search of InfoBlox documentation
"""
        ...
        try:
            ...
            if results['documents'] and len(results['documents'][0]) > 0:
                ...
                for i, (doc, metadata) in enumerate(zip(results['documents'][0], results['metadatas'][0])):
                    context_parts.append(f"[Knowledge Base {i+1}]\n{doc.strip()}")
            ...
            print(f"{Colors.BRIGHT_YELLOW}⚠ RAG search error: {e}{Colors.RESET}")
            return ""
...
def print_header():
    ...
    print(f"{Colors.BOLD}{Colors.BRIGHT_WHITE}{'🤖 Claude Sonnet 4.5 - DDI Assistant with RAG':^{width}}{Colors.RESET}")
    print(f"{Colors.BRIGHT_BLUE}{'AI Chat with InfoBlox Knowledge + Web Search + System Access':^{width}}{Colors.RESET}")
    print(f"{Colors.BOLD}{Colors.BRIGHT_CYAN}{'═' * width}{Colors.RESET}")
    print()
    print(f"{Colors.BRIGHT_GREEN}  Capabilities:{Colors.RESET}")
    print(f"{Colors.BRIGHT_YELLOW}    🧠{Colors.RESET} RAG-enhanced InfoBlox knowledge")
    ...
    if rag_manager.available:
        print(f"{Colors.BRIGHT_GREEN}  ✓ RAG Knowledge Base: ACTIVE{Colors.RESET}")
    else:
        print(f"{Colors.BRIGHT_YELLOW}  ⚠ RAG Knowledge Base: Not available{Colors.RESET}")
        print(f"{Colors.DIM}    Run: ./infoblox-rag-builder.py to build it{Colors.RESET}")
    print()
...
def print_rag_context():
    print(f"{Colors.DIM}{Colors.BRIGHT_BLACK}  (consulting knowledge base...){Colors.RESET}", end='', flush=True)
...
# Tool functions (same as before, plus RAG integration)
...
def infoblox_query(object_type, filters=None, max_results=100):
...
def get_all_tools():
    ...
    return [
        ...
        # InfoBlox tools
        {
            ...
            "description": "List networks from InfoBlox WAPI. The system has extensive knowledge about InfoBlox operations from its RAG knowledge base.",
            "input_schema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": "Get a specific network object by its reference (_ref)",
            "input_schema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "infoblox_query",
            "description": "Generic InfoBlox WAPI query for any object type. Use for advanced queries, including filtering by extensible attributes (EAs). For EA queries, use filters with *<EA_NAME> format (e.g., {'*MARSHA': 'HDQTR2'}). Always include '_return_fields+': 'extattrs' in filters when querying by EAs to see the EA values in results.",
            "input_schema": {
                "type": "object",
                "properties": {
                    "object_type": {"type": "string", "description": "InfoBlox object type (e.g., network, zone_auth, fixedaddress, range)"},
                    "filters": {"type": "object", "description": "Filter criteria as key-value pairs. For extensible attributes, use *<EA_NAME> format (e.g., {'*MARSHA': 'HDQTR2', '_return_fields+': 'extattrs'})"},
                    "max_results": {"type": "integer", "default": 100}
                },
...
def enhance_prompt_with_rag(user_message: str) -> str:
    ...
    if not rag_manager.available:
        return user_message

    # Search for relevant context with more results for complex queries
    n_results = 5 if any(word in user_message.lower() for word in ['where', 'filter', 'search', 'find']) else 3
    context = rag_manager.search(user_message, n_results=n_results)

    if context:
        # Prepend context to user message with explicit tool selection guidance
        enhanced = f"""[INFOBLOX KNOWLEDGE BASE CONTEXT]
...
1. Read the knowledge base context above carefully - it contains tool selection guidance and query patterns
2. If the query involves extensible attributes (EA) like "where MARSHA='HDQTR2'":
   - Use the appropriate infoblox_search_* tool (NOT list tool)
   - Add EA filters using *<EA_NAME> format in filters parameter
   - Include "extattrs" in return_fields to show the EAs in results
3. For complex queries requiring multiple steps, chain tools as shown in the examples
4. Always construct proper filters parameter for search queries
5. Use the exact tool names and parameter formats shown in the knowledge base

Provide a complete and actionable response using the InfoBlox tools available."""
        return enhanced
...
def main():
    logger.info("Starting Claude Chat RAG interface")
    ...
    while True:
        ...
        if rag_manager.available and any(word in user_input.lower() for word in ['infoblox', 'network', 'dns', 'dhcp', 'wapi', 'create', 'how']):
            print_rag_context()
